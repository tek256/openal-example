	...
	if(processed > 0){
		for(int i=0;i<processed;++i){
			...
			load_data:
				...
				bytes_used = stb_vorbis_decode_frame_pushdata(vorbis, buffer->data + buffer->offset, frame_size, &num_channels, &out, &num_samples);
				if(bytes_used == 0){
...
int main(int argc, char** argv){	
	...
	if(buffer.file_length - buffer.offset < BUFFER_SIZE){
		has_remaining = 0;	
	}
	...
	q = 1;
retry_init:
	v = stb_vorbis_open_pushdata(buffer.data, MAX_FRAME_SIZE, &used, &error, NULL);
	if(v == NULL){
...
initialize_buffer:
		bytes_used = stb_vorbis_decode_frame_pushdata(v, buffer.data + buffer.offset, data_length, &num_channels, &out, &num_samples);
